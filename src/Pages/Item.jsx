      ...
      <div className="border px-3 py-2 mb-2" key={categoryName}>
        ...
        <div className="flex justify-start overflow-x-scroll webkit w-full gap-3 py-2">
          {categoryItems.map((val, ind) => (
            <div
              key={ind}
              className="flex flex-col hover:rounded hover:shadow-xl hover:bg-transparent hover:transition-[0.2s] hover:scale-[1.03] p-1 items-center gap-2 w-[150px] sm:w-[160px]"
            >
                <img
                  src={val.image}
                  alt={val.name}
