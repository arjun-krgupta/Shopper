 ...
 <div className="md:px-10 lg:px-16 xl:px-28">
     <div className="border px-3 py-2 mb-2" key={categoryName}>
      <div className="flex justify-between px-2 mb-2">
        ...
        <Link to={`/${categoryName}`} className="text-sm hover:text-green-600">
          View all
        </Link>
      </div>
      <div className="flex justify-start overflow-x-scroll webkit w-full gap-3 py-1">
        {categoryItems.map((val, ind) => (
          <div
            key={ind}
            className="flex flex-col border rounded shadow-md p-1 items-center gap-[5px] w-[155px] sm:w-[170px] hover:border-[#9be15d]"
          >
            <div className="max-w-full w-auto h-[180px] p-1 cursor-pointer">
              <img
                src={val.image}
                alt={val.name}
